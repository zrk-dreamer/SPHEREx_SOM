...
class MiniSom(object):
    ...
    def __init__(self, x, y, input_len, sigma=1.0, learning_rate=0.5,
        ...
        distance_functions = {'euclidean': self._euclidean_distance,
                              ...
                              'chi2_distance': self._chi2_distance}
    ...
    def _activate(self, x, err):
        """Updates matrix activation_map, in this matrix
           the element i,j is the response of the neuron i,j to x."""
        self._activation_map = self._activation_distance(x, err, self._weights)

    def activate(self, x):
        """Returns the activation map to x."""
        self._activate(x, err)
    ...
    def _cosine_distance(self, x, err, w):
        num = (w * x).sum(axis=2)
        denum = multiply(linalg.norm(w, axis=2), linalg.norm(x))
        return 1 - num / (denum+1e-8)

    def _euclidean_distance(self, x, err, w):
        return linalg.norm(subtract(x, w), axis=-1)

    def _chi2_distance(self, x, err, w):
        return linalg.norm(subtract(x, w) / err, axis = -1)

    def _manhattan_distance(self, x, err, w):
        return linalg.norm(subtract(x, w), ord=1, axis=-1)

    def _chebyshev_distance(self, x, err, w):
    ...
    def train(self, data, errs, num_iteration,
        ...
        for t, iteration in enumerate(iterations):
            ...
            self.update(data[iteration], self.winner(data[iteration], errs[iteration]),
    ...
    def activation_response(self, data, errs):
        """
    ...
    def win_map(self, data, errs, return_indices=False):
        ...
        for i, x in enumerate(data):
            err = errs[i]
            winmap[self.winner(x, err)].append(i if return_indices else x)
        return winmap

    def labels_map(self, data, errs, labels):
        ...
        winmap = defaultdict(list)
        for x, l, e in zip(data, labels, errs):
            winmap[self.winner(x, e)].append(l)
